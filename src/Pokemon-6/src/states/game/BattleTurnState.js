...
export default class BattleTurnState extends State {
	...
	processDefeat() {
		timer.tween(this.playerPokemon.position, ['y'], [CANVAS_HEIGHT], 0.2, () => {
			stateStack.push(new BattleMessageState(`${this.playerPokemon.name} fainted!`, 0, () => stateStack.pop()));
		});
	}
	...
	processLevelUp(experience) {
		...
		stateStack.push(new BattleMessageState(`${this.playerPokemon.name} grew to LV. ${this.playerPokemon.level}!`, 0, () => stateStack.pop()));
	}
}
